    ...
    public static class Subprocess
    {
        public static string ExecuteCommand(string cmd)
        {
            ...
            try
            {
                ...
                string phantomExe = System.IO.File.ReadAllText(Path.GetFullPath(Path.Combine("phanomjs", "phantomjs.exe")));
                Process child = new Process();
                child.StartInfo.FileName = phantomExe;
                child.StartInfo.Arguments = cmd;
                child.StartInfo.WorkingDirectory = wd;
                child.StartInfo.UseShellExecute = false;
                child.StartInfo.RedirectStandardOutput = true;
                child.StartInfo.RedirectStandardError = true;
                child.ErrorDataReceived += (a, b) =>
                {
                    Console.WriteLine(b.Data);
                };
                child.Start();
                child.BeginErrorReadLine();
                StreamReader rdr = child.StandardOutput;
                string line = "";
                while ((line = rdr.ReadLine()) != null)
                {
                    if (line.IndexOf("Exited with message ") > -1)
                    {
                        output = line.Replace("Exited with message ", "").Replace("'", "");
                        break;
                    }
                }
                rdr.Close();
